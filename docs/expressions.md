...
相同时间标签，继续查询时使用 cycle 索引。请求触及未知或次序不明的时钟时会
明确拒绝；位于它之前的完整前缀仍可查询。`clock_edges_complete=false` 表示
全局时钟索引只有可信前缀，`total_edges_found` 是下限，不能据此判断波形已结束。

## 类型与精确绑定

输入字段有多处错误时，返回有界 `issues` 清单（最多 16 项），每项包含
`parameter` 和 `message`。一次修正已列出的字段后再重试；它只汇总字段校验问题，
类型求解、信号存在性和实际 dump 覆盖仍按需检查，不代表所有运行问题已经排除。
