...
For load queries, NPI first returns `net.load_list()` direct consumers. It uses
the broader terminal-register fan-out walk only when NPI reports no direct
handles; filters never trigger that expansion. This distinction matters for
global clock/reset/scan nets, where a full combinational cone may cover most of
an SoC even though the useful direct-load list is small.
