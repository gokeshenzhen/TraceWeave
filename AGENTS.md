...
changing environment/configuration. The compile log, source/include inputs,
TraceWeave installation, staging directory, and `TRACEWEAVE_CACHE_DIR` (including
the generated KDB) must be visible at the same absolute paths on submission and
...
`actual_backend="verdi_npi"`; otherwise inspect `fallback_reason`. On an
Xcelium cache miss, also run `build_kdb` and confirm its top-level execution
receipt reports the same LSF completion labels.
...
- `explain_signal_driver`, `find_signal_loads`, `trace_signal_path`, and
  ...
  one backend. For `trace_x_source`, one proved artifact serves every driver
  lookup in an attempt. A newly encountered X-bearing target outside that
  artifact triggers an exact hierarchy-ancestor-union rebuild and a restart
